...
\begin{itemize}
  ...
  \item Create two seeing data sets for LSST observing nights by
    ...
    random data derived from a time series model of the DIMM data.
  \item Create two additional seeing data sets for LSST observing
    ...
    filtered data following the same procedure applied to unfilterered
    data.
...
Figures~\ref{fig:decam-monthly-dimm-vs-year} and~\ref{fig:gemini-monthly-dimm-vs-year} shows the variation in
...
Figure~\ref{fig:gemini-vs-dimm} compares the seeing quantiles for
different months of DIMM and Gemini South IQ data. The correspondence
is worst when the seeing is very poor (and so is unlikely to be useful
in any case). Figure~\ref{fig:gemini-vs-dimm-hexbin} compares corresponding hourly means, and shows a similarly close correspondence.
...
\begin{figure*}
  ...
  \caption{ Color represents counts of hours in hexogonal bins in
...
et al 2006). To avoid using potentially bad data, the legacy seeing
database used by \texttt{opsim} (current as of October 2019) uses a
seeing database artificially generated using statistics derived from
Pachon DIMM data taken between 2004-05-06 and 2006-01-20, after a cuts
on data with low Strehl ratios in the left star
image. Figure~\ref{fig:claver-strehl-cut} shows the data used to
generate this database. The sharp cutoff in Strehl ratios indicate
...
quality: it may also be low due to atmospheric seeing
itself. Figure~\ref{fig:strehl-boxplots} shows both of these effects:
the upper panel shows that DIMM and Gemini South seeing are well
matched except when the Strehl ratio falls below 0.25, where the DIMM
shows significantly wider PSF FWHMs than the corresponding Gemini
...
any effect will be minor. Seeing simulations based on cut and uncut
DIMM data will be used to indicated the range.
...
The best developed methodologies for modeling time series naturally result in normal distributions: if we can transform the data set to be modeled to roughly match a normal distribution, then a wider variety of tools are available. The FWHM, as reporded by the DIMM, has a highly skewed distribution, with a long poor seeing tail, and a sharp limit to the good seeing. One physically meaningful quantity that can be mapped to the seeing is the Fried parameter, calculated for each reported DIMM value by inverting equation~5 of \cite{2002PASP..114.1156T}.
Figure~\ref{fig:r0-dist} shows the
distribution of measured values for the DIMM seeing (FWHM arcseconds) and
