...
def compute_seeing(
    ...
    Args:
        ...
        sample_coeff: AR1 model coefficient for sample variation
            sample_innovation: amplitude of sample model
            variation in log10(r0)
    ...
    >>> seeing_generator = compute_seeing(61100.0, 61101.0, 300,
    ...                           20,
    ...
    if nightly_offsets is None:
        nightly_offsets = ar1(
            nightly_coeff, nightly_innovation, init_nightly_offset
        )
    ...
    for nightly_offset in nightly_offsets:
        ...
        night_log_r0 = (
            mean_log_r0 + seasonal_offset(mjd + 0.5) + nightly_offset
        )
        ...
        for sample_offset in sample_offsets:
            ...
            r0 = np.power(10, log_r0)  # pylint: disable=invalid-name
            ...
            delta_mjd = elapsed_seconds - start_elapsed_seconds
            mjd = start_mjd + delta_mjd / (24.0 * 60.0 * 60.0)


def sim_seeing(stream=sys.stdout, first=False, **kwargs):
    """Generate artificial seeing and write it to a file.

    Args:
        stream: the file pointer to the file to write

    The remaining arguments are as in simsee.seeing
    """
    writer = csv.writer(stream, delimiter="\t")
    for seeing_record in compute_seeing(**kwargs):
        if first:
...
def interpolate_seeing(dimm, stream=sys.stdout, **kwargs):
    ...
    Args:
       ...
       stream: the file name or pointer to the output data set
    ...
    if CONSTANT_MEAN:
        mjd_offset = int(round(YEAR_LENGTH_DAYS * years_offset))
        mjds = np.arange(
            calc_night_mjd(start_mjd) - mjd_offset - 1,
            calc_night_mjd(end_mjd) - mjd_offset + 2,
        )
        longterm_fit_log_r0 = pd.DataFrame(
            {"night_mjd": mjds, "fit_diff": mean_log_r0}
        ).set_index("night_mjd", drop=False)
    else:
        mjd_offset = int(round(YEAR_LENGTH_DAYS * years_offset))
        mjds = np.arange(
            calc_night_mjd(start_mjd) - mjd_offset - 1,
        ...
        dimm_nights = (
            dimm.groupby("night_mjd")
            .agg({"log_r0": "mean"})
            .reset_index(drop=False)
        )
        seasonal_diff = dimm_nights.log_r0 - year_cos(
            dimm_nights.night_mjd, seasonal_phase, seasonal_amplitude
        )
        calc_longterm_diff = UnivariateSpline(
            dimm_nights.night_mjd.values,
            seasonal_diff,
            k=2,
            s=0.012 * len(dimm_nights),
            ext=3,
        )
        longterm_fit_log_r0 = pd.DataFrame(
            {"night_mjd": mjds, "fit_diff": calc_longterm_diff(mjds)}
        ).set_index("night_mjd", drop=False)
        longterm_fit_log_r0.to_csv("longterm_fit_log_r0.txt", sep="\t")
        logging.info(
            "Long term fit params: %s",
            str(longterm_fit_log_r0.fit_diff.describe()),
        )

    nightly_dimm = interpolate_night_seeing(
        dimm,
    ...
    dimm_in_time = dimm.query(
        "{0} < mjd < {1}".format(start_mjd - mjd_offset, end_mjd - mjd_offset)
    )
    dimm_in_time = dimm_in_time.copy()
    dimm_in_time["elapsed_seconds"] = np.round(
        (dimm_in_time.mjd + mjd_offset - start_mjd) * 24 * 60 * 60
    ).astype(int)
    ...
    for dimm_time, dimm_row in dimm_in_time.iterrows():
        ...
        night_date = pd.to_datetime(
            next_mjd + 2400000, unit="D", origin="julian"
        )
        month_name = night_date.month_name()
        if month_name != printed_month:
            logging.info(
                "Creating %s, %d", night_date.month_name(), night_date.year
            )
        ...
        if next_mjd > prev_mjd + freq_days:
            ...
            try:
                init_sample_offset = (
                    prev_log_r0 - nightly_dimm[sim_start_night]
                )
            except UnboundLocalError:
                # This is the first point
            ...
            nightly_offsets = (
                nightly_dimm.loc[sim_start_night:sim_end_night].values
                - seasonal_offset(
                    0.5
                    + nightly_dimm.loc[
                        sim_start_night:sim_end_night
                    ].index.values
                )
                - mean_log_r0
            )
        ...
        sample_seeing = SeeingSample(
            next_mjd,
            int(dimm_row.elapsed_seconds),
            dimm_row.r0,
            dimm_row.vk_seeing,
            dimm_row.seeing,
            dimm_time.isoformat(),
...
def main():
    ...
    parser = ArgumentParser(
        description="Generate a simulated seeing data set for survey strategy simulation."
    )
    parser.add_argument(
        "config_fname", type=str, help="file with configuration parameters"
    )
    ...
    if "dimm_fname" in config:
        dimm = load_dimm(
            config["dimm_fname"], outer_scale=config["outer_scale"]
        )
        interpolate_seeing(dimm, output_stream, **config)
    else:
        sim_seeing(output_stream, True, **config)
...
def vk_seeing(
    r0, outer_scale=20.0, wavelength=5.0e-7
...
def interpolate_night_seeing(
    ...
    for mjd in range(start_mjd, end_mjd + 1):
        ...
        this_longterm_fit_log_r0 = longterm_fit_log_r0.loc[
            dimm_mjd, "fit_diff"
        ]
        try:
            log_r0 = dimm_nights.loc[dimm_mjd, "log_r0"]
            nightly_offset = (
                log_r0 - this_longterm_fit_log_r0 - expected_seasonal_offset
            )
        except KeyError:
            nightly_offset = nightly_coeff * nightly_offset + np.random.normal(
                0.0, nightly_innovation
            )
            log_r0 = (
                nightly_offset
                + this_longterm_fit_log_r0
                + expected_seasonal_offset
            )
        mjds.append(mjd)
...
def year_cos(mjd, seasonal_phase, seasonal_amplitude):
    ...
    angle = (
        (mjd - mjd_jan_1_2000 - seasonal_phase) * 2 * np.pi / YEAR_LENGTH_DAYS
    )
